...
class TimePeriod:
    ...
    def __contains__(self, other: Union[dt.date, "TimePeriod"]) -> bool:
        if isinstance(other, dt.date):
            return self.start <= other <= self.end
        return self.start <= other.start <= other.end <= self.end

    def is_overlapping_lower_range(self, other: "TimePeriod") -> bool:
        return other.start in self and other.end not in self

    def is_overlapping_upper_range(self, other: "TimePeriod") -> bool:
        return other.end in self and other.start not in self

    def intersection(self, other: "TimePeriod") -> dt.timedelta:
        """
